...
def model_fn_w_pruning(features, labels, mode, params):
  ...
  if task in ["pie_dataset_gen", "imagenet_training", "imagenet_predictions", "cifar10_training"]:
  ...
  elif params['model_name'] == 'wide_resnet':
      network = widenet_model.wide_resnet(
          num_classes=params["num_label_classes"],
          depth=params['widenet_depth'],
          width=params['widenet_width'],
          data_format="channels_last")
  else:
      raise ValueError('Invalid model name: {}'.format(params['model_name']))
...
def initiate_task_helper(model_params,
  ...
  """
  if model_params['dataset'] == 'imagenet':
      input_fn = data_input.input_fn
  elif model_params['dataset'] == 'cifar10':
      input_fn = data_input_cifar10.input_fn
  else:
      raise ValueError('Invalid dataset name: {}'.format(model_params['dataset']))

  if model_params["task"] != "imagenet_training" and model_params["task"] != "cifar10_training":
    ...
    if model_params["task"] in ["imagenet_predictions"]:
      predictions = classifier.predict(
          input_fn=input_fn, checkpoint_path=ckpt_directory)
      return predictions
    ...
    classifier.train(
        input_fn=input_fn, max_steps=model_params["num_train_steps"])
